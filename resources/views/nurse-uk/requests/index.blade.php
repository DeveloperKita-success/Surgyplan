    ...
    <div class="space-y-6">
        <div class="flex flex-wrap gap-2">
            @foreach ([
                ...
                <a
                    href="{{ $filter['value'] ? route('nurse-uk.requests.index', ['status' => $filter['value']]) : route('nurse-uk.requests.index') }}"
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($requests as $request)
                            <tr>
                                ...
                                <td class="px-5 py-4">
                                    <a href="{{ route('nurse-uk.requests.show', $request) }}" class="rounded-md border border-slate-200 px-3 py-1.5 font-medium text-slate-700 hover:bg-slate-50">
                                        Periksa
                                    </a>
                                </td>
                            </tr>
