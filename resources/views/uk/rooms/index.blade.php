<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <form method="GET" action="{{ route('uk.rooms.index') }}" class="grid gap-3 lg:grid-cols-[minmax(240px,1fr)_180px_auto]">
                <input
                    type="search"
                    name="q"
                    value="{{ $query }}"
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('uk.rooms.index') }}" class="rounded-lg border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">Reset</a>
                </div>
            </form>

            <a href="{{ route('uk.rooms.create') }}" class="rounded-lg bg-cyan-700 px-4 py-2.5 text-sm font-semibold text-white hover:bg-cyan-800">
                Tambah Kamar
            </a>
        </div>
                            ...
                            <tr>
                                ...
                                <td class="px-5 py-4">
                                    <div class="flex justify-end gap-2">
                                        <a href="{{ route('uk.rooms.show', $room) }}" class="rounded-md border border-slate-200 px-3 py-1.5 font-medium text-slate-700 hover:bg-slate-50">Detail</a>
                                        <a href="{{ route('uk.rooms.edit', $room) }}" class="rounded-md border border-cyan-200 px-3 py-1.5 font-medium text-cyan-700 hover:bg-cyan-50">Edit</a>
                                        <form method="POST" action="{{ route('uk.rooms.destroy', $room) }}" onsubmit="return confirm('Hapus kamar operasi ini?');">
                                            @csrf
                                            @method('DELETE')
