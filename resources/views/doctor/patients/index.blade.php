        ...
        <table class="min-w-full divide-y divide-slate-200 text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($schedules as $schedule)
                    <tr><td class="px-5 py-4 font-medium">{{ $schedule->patient->name }}</td><td class="px-5 py-4">{{ $schedule->surgeryRequest->procedure_text ?? '-' }}</td><td class="px-5 py-4">{{ $schedule->surgery_date?->format('d M Y') }}</td></tr>
                @empty
